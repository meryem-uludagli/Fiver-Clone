...
export const getGig = c(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    res.status(200).json({ message: "Hizmet verisi alindi" });
  }
);
...
export const deleteGig = c(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (gig?.user !== req.userId)
      return next(error(403, "Bu islemi yapmaya yetkiniz yok"));
    await Gig.findByIdAndDelete(req.params.id);
    res.status(200).json({ message: "hizmet basariyla kaldirildi" });
  }
);
